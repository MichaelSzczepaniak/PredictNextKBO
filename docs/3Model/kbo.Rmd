...
date: "April 28, 2016 (last updated)"
...
Before moving on with the rest of the development, let's look closely at these two equations and make sure that they make sense.  The right hand sides of both 14. and 15. start with 1 because the total mass (aka area under the probablity mass curve) is 1, so this is the total amount of probability mass.  The second term is the probability mass which remains after the discount has been taken.  So the difference is the amount of probability mass taken from the original distribution which must now we distributed to the unknown bigrams as in 14. or trigrams as in 15..

At this point, we've finished the first step by defining the amount of probability mass taken from the observed bigrams and trigrams as a function of some discounted amount $\gamma_{2}$ or $\gamma_{3}$.  Now we need to know how to assign this discounted probability mass to unseen bigrams and trigrams.  Katz proposed that this be done proportionally to the backed-off (n-1)-gram probability parameters.  In other words, an unobserved trigram probability would be estimated by distributing $\alpha(w_{i-2},\:w_{i-1})$ in proportion to all the possible bigram tails and the unobserved bigram probabilities would be estimated by distributing $\alpha(w_{i-1})$ in proportion to all the possible unigram tails.
...
respectively, where the terms $w\:\in\:\mathcal{B}(w_{i-1})$ and $w\:\in\:\mathcal{B}(w_{i-2},\:w_{i-1})$ refer to all the words *w* in the set $\mathcal{B}$ which are all words that terminate an **unobserved** bigram starting with $(w_{i-1})$ or **unobserved** trigram starting with $(w_{i-2},\:w_{i-1})$ respectively.  Notice that the subscript in the middle $q$ term of equations 16. is **ML**.  This is because at this point, we are using the Maximum Likelihood estimate for the unigrams which allows for the simplification to the counts shown in the rightmost term.
