...
where the terms $w\:\in\:\mathcal{B}(w_{i-1})$ and $w\:\in\:\mathcal{B}(w_{i-2},\:w_{i-1})$ refer to all the words *w* in the set $\mathcal{B}$ which are all words that terminate an **unobserved** bigram starting with $(w_{i-1})$ or trigram starting with $(w_{i-2},\:w_{i-1})$ respectively.

#### Steps in Applying the Alogrithm

1. Create table of unigram, bigram, and trigram counts.
2. Select the values for $\gamma_2$ and $\gamma_3$.
3. tbd
4. tbd
5. tbd
6. tbd
7. Select $w_i$ with the highest $q_{BO}(w_i\:|\:w_{i-2},\:w_{i-1})$ as the prediction.
