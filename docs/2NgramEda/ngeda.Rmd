...
[en_US.news.train.9rawunig.csv](https://www.dropbox.com/s/4u90r188ht2etic/en_US.news.train.9rawunig.csv?dl=1)  | `r round(fsws[2], 2)` | [en_US.news.train.12unigrams.nosins.csv](https://www.dropbox.com/s/uo971onngv468t1/en_US.news.train.12unigrams.nosins.csv?dl=0) | `r round(fsns[2], 2)` | `r round(diff[2], 2)`
...
tbd   | `r round(fsws[4], 2)` | tbd | `r round(fsns[4], 2)` | `r diff[4]`
tbd   | `r round(fsws[5], 2)` | tbd | `r round(fsns[5], 2)` | `r diff[5]`
tbd   | `r round(fsws[6], 2)` | tbd | `r round(fsns[6], 2)` | `r diff[6]`
tbd   | `r round(fsws[7], 2)` | tbd | `r round(fsns[7], 2)` | `r diff[7]`
tbd   | `r round(fsws[8], 2)` | tbd | `r round(fsns[8], 2)` | `r diff[8]`
tbd   | `r round(fsws[9], 2)` | tbd | `r round(fsns[9], 2)` | `r diff[9]`
Totals | `r round(sum(fsws), 2)`|   | `r round(sum(fsns), 2)`| `r round(sum(diff), 2)`


The current free plan on [shinyapps.io](http://www.shinyapps.io/) limits deployments to [1 Gb of memory for a large instance](http://shiny.rstudio.com/articles/shinyapps.html) which quite generous.  However, as the table below shows, our n-gram tables are too large if we include singletons.



The lessons around the importance of dealing with singletons came from discovering that trying to build just a portion of a bigram frequency tables took an extraordinary amount of time (multiple days even on a quad-core i7 with 16 Gb of RAM) to complete.  One way to speed up this computationally expensive processing was to replace each unigram singleton with a special token before trying to build the bigram or trigram frequency tables.

The workflow went as follows:

1. Create lists of unigram singletons from each of the three **en_US.\*.train.9rawunig.csv** files (links provided above).  The **removeSingletons** function was used to generate the [en_US.blogs.train.10singles.txt](https://www.dropbox.com/s/ttbxocp0u7kw5u9/en_US.blogs.train.10usingles.txt?dl=1), [en_US.news.train.10singles.txt](https://www.dropbox.com/s/fyuxceuv8m318rd/en_US.news.train.10usingles.txt?dl=1) and [en_US.twitter.train.10singles.txt](https://www.dropbox.com/s/gcu4lw15zsx482q/en_US.twitter.train.10usingles.txt?dl=0) files.
2. Create revised (singletons removed) unigram frequency tables.  The **removeSingletons** function was used to generate the [en_US.blogs.train.11unigrams.nosins.csv](https://www.dropbox.com/s/033qzeiggmcauo9/en_US.blogs.train.11unigrams.nosins.csv?dl=0), [en_US.news.train.11unigrams.nosins.csv](https://www.dropbox.com/s/uo971onngv468t1/en_US.news.train.11unigrams.nosins.csv?dl=0) and [en_US.twitter.train.11unigrams.nosins.csv](https://www.dropbox.com/s/41yzwgwdviv8i5k/en_US.twitter.train.11unigrams.nosins.csv?dl=0) files as well.
3. Replace each unigram singleton in each of the **en_US.\*.train.8posteos.txt** with the designated **USIN** token.  Because this was a very computationally expensive step, it was broken into several sub-steps which are described in the **TokenizeFromList** project in my git repository.  The results of this step produced the following three files: [en_US.blogs.12ustokens.txt](), [en_US.news.12ustokens.txt]() and [en_US.twitter.12ustokens.txt]()
...
makeRawNgrams <- function(table.dir=ddir, filePrefix="en_US.",
    ...
    for(i in 1:length(inPaths)) {
        ...
        unigrams.raw <- getNgramTables(n, charvect)
