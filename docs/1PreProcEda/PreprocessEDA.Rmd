---
title: "Predicting Next Word Using Katz Back-Off: Part 1 - Overview and Preprocessing"
...
This document describes the details around how the data was partitioned and how it was cleaned prior to n-gram table construction and exploratory data analysis (EDA) was done.  This was the most time consuming step which was no surprise given [what some of the literature reports.](https://www.jstatsoft.org/index.php/jss/article/view/v059i10/v59i10.pdf)
...
Once the data was cleaned, unigram, bigram, and trigram frequency tables were constructed.  These tables are at the heart of the data required by the model to make its predictions.  Some EDA was performed on with these tables to gain insights which would later serve the model development phase.
...
3. Unicode Tag Conversions and Filtering - The initial reading of the data in the partitioning step generated unicode tags which needed to be processed to avoid model accuracy degradation.
4. URL Filtering - An extensive number of URL's were found in the corpus documents.  These are artifacts that the model was not designed to predict, so they were removed.  Because these URL's took on a number of forms (e.g. http:\\..., www.example.com, etc) the filtering processing focused on removing a majority of these fragments.
...
6. Add EOS Markers - A special EOS token replaced all markes that typically end sentences (e.g. periods, question marks, etc.)
7. Post-EOS Marker Filtering - The pre-EOS and add-EOS marker steps generated additional fragments that needed to be cleaned up.  This step did this processing.
...
The **addEosMarkers** function was passed to **runFilterAndWrite** to do the EOS marker insertion.  The outputs from this function resulted in the following 3 files:
