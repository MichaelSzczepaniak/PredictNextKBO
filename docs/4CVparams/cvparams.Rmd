...
#### Step 3 - Create the N-Gram Frequency Tables for Each Folds Training Set
...
#### Step 4 - Select the $(\gamma_2, \gamma_3)$ Values to Run Against the Test Set
...
#### Step 5 - Build the Set of Bigrams to Use as Model Inputs

To assess the accuracy of the model, we need to give it a set of bigrams, have the model predict the words that follow each and then deterimine how accurate the predictions were.  We do this by selecting 500 random trigrams from the test set (depicted in gold), parsing out the first two words to use as input to the prediction model and then using the last word to determine if the model predicted accurately or not.  The random trigrams for each fold were created and written to files using the **makePredictTrigrams** function and these trigrams are in files which are listed in the table below.

Fold | Blogs Prediction Trigrams |
-----|-----------:|-----------:
1    | [fold_1blogs_predict.txt](https://www.dropbox.com/s/ow47mbpakiuhk0l/fold_1blogs_predict.txt?dl=1)
2    | [fold_2blogs_predict.txt]()
3    | [fold_3blogs_predict.txt]()
4    | [fold_4blogs_predict.txt]()
5    | [fold_5blogs_predict.txt]()

#### Step 6 - Run the Model to Predict the Tail Words of Each Folds Set of Random Trigrams

For each fold, we select a pair of discount rates $(\gamma_2, \gamma_3)$, make 500 predictions, and count the number of accurate predictions.  We then select another pair of the 361 combinations of $(\gamma_2, \gamma_3)$, make another 500 predictions, and assess the accuracy.  The function **trainFold** repeats this process for a single fold until all combinations of $(\gamma_2, \gamma_3)$ pairs have been run.  Running 500 predictions for each of the 361 combinations of $(\gamma_2, \gamma_3)$ in each of the 5 folds was a very time consuming process which is why only the results for the **Blogs** corpus are being reported here.  The results for each fold were written to the files listed in the table below.

Fold | Blogs Results Files | Best Train Accuracy | $(\gamma_2, \gamma_3)$ count 
-----|--------------------:|--------------:|-----------------------------:|
1    | [cv_blogs_fold1_itrs500.csv](https://www.dropbox.com/s/524dgfw0ej2d4m3/cv_blogs_fold1_itrs500.csv?dl=1) | 0.196 | 24
2    | [cv_blogs_fold2_itrs500.csv]() | 0.184 | 152
3    | [cv_blogs_fold3_itrs500.csv]() | 0.184 | 90
4    | [cv_blogs_fold4_itrs500.csv]() | 0.216 | 19
5    | [cv_blogs_fold5_itrs500.csv]() | 0.212 | 82

#### Step 7 - Run the Best $(\gamma_2, \gamma_3)$ Values Against the Validation Set

From the table in **Step 6**, we see that a number of $(\gamma_2, \gamma_3)$ combinations resulted in the highest accuracy on the test set.  Each of these pairs were obtained by running the **getBestDiscountPairs** function and then written to the files listed in the **Best Blog Training** column of the table below.  These best training pairs were then run against the validation set by running the **getValidationResults** function and the highest accuracy was found to be the values reported in the **Best Validation Accuracy** column of the table below.

Fold | Best Blog Training | Best Validation Accuracy | $(\gamma_2, \gamma_3)$ count
-----|-------------------:|--------------:|-----------------------------:|
1    | [fold1_best_train.csv]() | TBD | TBD
2    | [fold2_best_train.csv]() | TBD | TBD
3    | [fold3_best_train.csv]() | TBD | TBD
4    | [fold4_best_train.csv]() | TBD | TBD
5    | [fold5_best_train.csv]() | TBD | TBD

### Selecting the Best Discount Pair

As we observed in **Step 6**, we see that a number of $(\gamma_2, \gamma_3)$ combinations resulted in the highest accuracy on the validation set...
...
getBestDiscountPairs <- function(result_file_paths=NULL) {
    ...
    if(is.null(result_file_paths)) {
        result_files_list[[1]] <- read.csv('https://www.dropbox.com/s/524dgfw0ej2d4m3/cv_blogs_fold1_itrs500.csv?dl=1')
        result_files_list[[2]] <- read.csv('https://www.dropbox.com/s/2k5ypy3sovn6g9d/cv_blogs_fold2_itrs500.csv?dl=1')
        result_files_list[[3]] <- read.csv('https://www.dropbox.com/s/85ex5o9km20014m/cv_blogs_fold3_itrs500.csv?dl=1')
        result_files_list[[4]] <- read.csv('https://www.dropbox.com/s/5rf7mbhrpq617e9/cv_blogs_fold4_itrs500.csv?dl=1')
        result_files_list[[5]] <- read.csv('https://www.dropbox.com/s/vxq9bl6bmagv1x2/cv_blogs_fold5_itrs500.csv?dl=1')
    } else {
