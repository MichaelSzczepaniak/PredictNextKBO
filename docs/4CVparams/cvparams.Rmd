...
A good way to start understanding this approach is to first visualize breaking the data into k roughly equal partitions.  For this analysis, k=10 was selected because we have quite a bit of data and our accuracy rate is the neighborhood of 20% which implies quite a bit of variability in our predictions.  The picture below illustrates the process.  

<img src="k=10fold_cv.png" alt="K-Fold Cross Validation" height=242 width=242/>

We train on the parts of the data represented by the blue boxes and validate on the parts of the data represented by the white boxes.  Details are described in the next section.
...
Each fold is represented as one of the ten rows shown in the picture above.  Random indices for lines in the **blogs** corpus were selected using the **makeFolds** function to create each of the folds in the data.  Within each fold there are 2 partitions.  The **validation partition** was created using the **makeValidation** function and is depicted by the white box in each fold (10% of the data in a fold).  The remaining non-validation data is considered the **training set** of each fold.  

Fold | Blogs Fold Partition | Blogs Training Set | Blogs Validation Set
-----|---------------:|--------------:|--------------:
1    | [fold_1blogs.txt]() |  |  |  
2    | [fold_2blogs.txt]() |  |  |  
3    | [fold_3blogs.txt]() |  |  |  
...
-----|---------------:|--------------:|--------------:
1    | [fold_1train_blogs_1grams.csv]() | [fold_1train_blogs_2grams.csv]() | [fold_1train_blogs_3grams.csv]() 
...
6    | [fold_6train_blogs_1grams.csv]() | [fold_6train_blogs_2grams.csv]() | [fold_6train_blogs_3grams.csv]() 
7    | [fold_7train_blogs_1grams.csv]() | [fold_7train_blogs_2grams.csv]() | [fold_7train_blogs_3grams.csv]() 
8    | [fold_8train_blogs_1grams.csv]() | [fold_8train_blogs_2grams.csv]() | [fold_8train_blogs_3grams.csv]() 
8    | [fold_9train_blogs_1grams.csv]() | [fold_9train_blogs_2grams.csv]() | [fold_9train_blogs_3grams.csv]() 
10    | [fold_10train_blogs_1grams.csv]() | [fold_10train_blogs_2grams.csv]() | [fold_10train_blogs_3grams.csv]() 

#### Step 3 - Select the $(\gamma_2, \gamma_3)$ Values to Run Against the Test Set

In addition to the n-gram tables, we also need to specify two parameters: a bigram discount rate, and a trigram discount rate.  These were the $\gamma_2$ and $\gamma_3$ variables described in [Part 3](http://rpubs.com/mszczepaniak/predictkbo3model).  We vary each discount rate from 0.1 to 2.9 in steps of 0.2.  This gives us a 15 x 15 = 225 combinations to train over.
...
```{r cache=TRUE}
## TODO **FILL IN THE PROPER LINKS TO THE ITERATION FILES WHEN THEY ARE READY!!**
...
getBestWorstResults <- function(fold_results_files=
                           c('https://www.dropbox.com/tbd/cv_blogs_fold1_itrs500.csv?dl=1',
                             'https://www.dropbox.com/tbd/cv_blogs_fold2_itrs500.csv?dl=1',
                             'https://www.dropbox.com/tbd/cv_blogs_fold3_itrs500.csv?dl=1',
                             'https://www.dropbox.com/tbd/cv_blogs_fold4_itrs500.csv?dl=1',
                             'https://www.dropbox.com/tbd/cv_blogs_fold5_itrs500.csv?dl=1',
                             'https://www.dropbox.com/tbd/cv_blogs_fold6_itrs500.csv?dl=1',
                             'https://www.dropbox.com/tbd/cv_blogs_fold7_itrs500.csv?dl=1',
                             'https://www.dropbox.com/tbd/cv_blogs_fold8_itrs500.csv?dl=1',
                             'https://www.dropbox.com/tbd/cv_blogs_fold9_itrs500.csv?dl=1',
                             'https://www.dropbox.com/tbd/cv_blogs_fold10_itrs500.csv?dl=1')) {
    library(dplyr)
...
# temporary place holders for TBD values
fold_train_worst <- rep('TBD',10)
fold_train_best <- rep('TBD',10)
fold_train_best_count <- rep('TBD',10)
```


Fold | Blogs Prediction Trigrams | Blogs Results Files | Worst Train Accuracy | Best Train Accuracy | Best $(\gamma_2, \gamma_3)$ count 
-----|-----------:|---------------:|--------:|--------:|--------:
1    | [fold_1blogs_predict.txt]() | [cv_blogs_fold1_itrs500.csv]() | `r fold_train_worst[1]` | `r fold_train_best[1]` | `r fold_train_best_count[1]`
2    | [fold_2blogs_predict.txt]() | [cv_blogs_fold2_itrs500.csv]() | `r fold_train_worst[2]` | `r fold_train_best[2]` | `r fold_train_best_count[2]`
3    | [fold_3blogs_predict.txt]() | [cv_blogs_fold3_itrs500.csv]() | `r fold_train_worst[3]` | `r fold_train_best[3]` | `r fold_train_best_count[3]`
4    | [fold_4blogs_predict.txt]() | [cv_blogs_fold4_itrs500.csv]() | `r fold_train_worst[4]` | `r fold_train_best[4]` | `r fold_train_best_count[4]`
5    | [fold_5blogs_predict.txt]() | [cv_blogs_fold5_itrs500.csv]() | `r fold_train_worst[5]` | `r fold_train_best[5]` | `r fold_train_best_count[5]`
6    | [fold_6blogs_predict.txt]() | [cv_blogs_fold6_itrs500.csv]() | `r fold_train_worst[6]` | `r fold_train_best[6]` | `r fold_train_best_count[6]`
7    | [fold_7blogs_predict.txt]() | [cv_blogs_fold7_itrs500.csv]() | `r fold_train_worst[7]` | `r fold_train_best[7]` | `r fold_train_best_count[7]`
8    | [fold_8blogs_predict.txt]() | [cv_blogs_fold8_itrs500.csv]() | `r fold_train_worst[8]` | `r fold_train_best[8]` | `r fold_train_best_count[8]`
9    | [fold_9blogs_predict.txt]() | [cv_blogs_fold9_itrs500.csv]() | `r fold_train_worst[9]` | `r fold_train_best[9]` | `r fold_train_best_count[9]`
10    | [fold_10blogs_predict.txt]() | [cv_blogs_fold10_itrs500.csv]() | `r fold_train_worst[10]` | `r fold_train_best[10]` | `r fold_train_best_count[10]`
...
# temporary place holders for TBD values
fold_valid_worst <- rep('TBD', 10)
fold_valid_best <- rep('TBD', 10)
fold_valid_best_count <- rep('TBD', 10)
```
...
-----|-----------:|-----------:|-----------:|-----------:|-----------:
1    | [fold_1valid_predict.txt]() | [fold1_best_train.csv]() | `r fold_valid_worst[1]` | `r fold_valid_best[1]` | `r fold_valid_best_count[1]`
2    | [fold_2valid_predict.txt]() | [fold2_best_train.csv]() |`r fold_valid_worst[2]` | `r fold_valid_best[2]` | `r fold_valid_best_count[2]`
3    | [fold_3valid_predict.txt]() | [fold3_best_train.csv]() |`r fold_valid_worst[3]` | `r fold_valid_best[3]` | `r fold_valid_best_count[3]`
4    | [fold_4valid_predict.txt]() | [fold4_best_train.csv]() |`r fold_valid_worst[4]` | `r fold_valid_best[4]` | `r fold_valid_best_count[4]`
5    | [fold_5valid_predict.txt]() | [fold5_best_train.csv]() |`r fold_valid_worst[5]` | `r fold_valid_best[5]` | `r fold_valid_best_count[5]`
6    | [fold_6valid_predict.txt]() | [fold6_best_train.csv]() |`r fold_valid_worst[6]` | `r fold_valid_best[6]` | `r fold_valid_best_count[6]`
7    | [fold_7valid_predict.txt]() | [fold7_best_train.csv]() |`r fold_valid_worst[7]` | `r fold_valid_best[7]` | `r fold_valid_best_count[7]`
8    | [fold_8valid_predict.txt]() | [fold8_best_train.csv]() |`r fold_valid_worst[8]` | `r fold_valid_best[8]` | `r fold_valid_best_count[8]`
9    | [fold_9valid_predict.txt]() | [fold9_best_train.csv]() |`r fold_valid_worst[9]` | `r fold_valid_best[9]` | `r fold_valid_best_count[9]`
10    | [fold_10valid_predict.txt]() | [fold10_best_train.csv]() |`r fold_valid_worst[10]` | `r fold_valid_best[10]` | `r fold_valid_best_count[10]`

### Selecting the Best Discount Pair

The highest accuracy was found in fold **TBD**, but **TBD** different $(\gamma_2, \gamma_3)$ pairs all yielded the same best accuracy of `r fold_valid_best[5]`.  The problem now shifts to deciding which of these **TBD** pairs should be selected.  The table in **Step 5** provides at least three useful clues.
...
## The 10 files in this vector were created by running the expandAllValidationResults
...
contour(x=xgrid, y=ygrid, comp_mat, xlab='gamma2', ylab='gamma3',
        main="THIS GRAPH IS NOT CORRECT - IT IS JUST A PLACEHOLDER")
```
...
p <- p + ggtitle("THIS GRAPH IS NOT CORRECT - IT IS JUST A PLACEHOLDER")
...
It appears that the discount "sweet spot" is in the lower right portion of the contour and heat maps.  Accuracy appeared to improve as trigram discount was decreased.  The region of highest overlap was the region where $0 < \gamma_2 = TBD$ and $TBD \geq \gamma_3 \leq TBD$.  Three of the folds needed to overlap in order to form this region.  The center of the sweet spot is approximately $(\gamma_2 =TBD, \gamma_3 = TBD)$ and is what we are selecting as our best values based on this analysis.
...
p <- p + geom_histogram(breaks=seq(0.134, 0.234, length.out = 9),
                        ...
                        labs(title= "THIS GRAPH IS NOT CORRECT - IT IS JUST A PLACEHOLDER") +
...
### Confidence Interval - VALUES SHOWN BELOW ARE NOT CORRECT: THEY CURRENTLY PLACEHOLDERS!!!
...
```{r eval=FALSE}
##
makeValidFolds <- function(
        fold_paths=c("https://www.dropbox.com/s/fo7ybvw4h7de2e8/fold_1blogs.txt?dl=1",
        ...
        corp_type='blogs', validFrac=0.2, seed_value=719) {
    fold <- readFolds(fold_paths, corp_type)
}
...
## vector of ints that are the indices read from the files located at each location
## specified in fold_paths, except for the last item which is the total line count
## of all the folds
