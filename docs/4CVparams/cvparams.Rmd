...
Three observations jump out from the table in **Step 5**: First, a number of $(\gamma_2, \gamma_3)$ combinations resulted in the highest accuracy on the validation set in this step as well.  Second, the difference between the worst and best accuracy within a valildation fold is either small or non-existant.  Third, the best training accuracy values are all rather similar.  Taken together, this suggests that a reasonable selection criteria would be to find the region of greatest overlap if one exists, and if it does, then take the $(\gamma_2, \gamma_3)$ that gives the highest validation accuracy within this region to select a single pair.

The first step is to find the overlap in the $(\gamma_2, \gamma_3)$ spaces.  Since each fold tests a different set of $(\gamma_2, \gamma_3)$ pairs against the validation set, we expand each of the validation results into the original 2D grid so that we can aggragate them easily which will make finding the overlap areas easier.  This was done by running the **expandAllValidationResults** function.  We then convert these to matices using the **convertDfToContourMatrix** function, add them up, and then pass the resulting matrix to **contour** to get a quick visual of the results.
...
Some (like me) find it easier to view this kind of data as a heatmap.  In the plot below, red is largest and white is zero.  Note also that zero in both the plot above and below are are just regions that were not selected in the final validation runs.
...
It appears that the discount "sweet spot" is in the lower right portion of the contour and heat maps.  Accuracy appeared to improve as trigram discount was decreased.  The region of highest overlap was the region where $0 < \gamma_2 = 0.2$ and $1.4 \geq \gamma_3 \leq 1.8$.  Three of the folds needed to overlap in order to form this region.  The center of the sweet spot is approximately $(\gamma_2 = 1.6, \gamma_3 = 0.1)$ and is recommended as the best value based on this analysis.

### Running Best Values Against Test Set

Now that we have selected our best values, it time to run them against the test data which the model has not seen.  As we did during training and validation, we'll select 500 random trigrams from the held out test set and do this 100 times so that we can get estimates for the test set accuracy and the variance of this estimate.

#### How Final Model was Constructed

As a reminder, the model is composed of three parts: an algorithm implementation (Katz Back-Off Trigram), three n-gram frequency tables (unigram, bigram, and trigram), and values for the two discount parameters (bigram and trigram).  We have the first and third parts, but need to construct the final version of the n-gram frequency tables before we can evaluate against the test set.

This construction was done by combining the n-gram tables from the three folds which overlapped to produce the final estimates of the discount parameters.  Those folds, identified by simple inspection, were 1, 2, and 3,

*TODO...*

### Confidence Interval

From the table above...  *to be continued...*
...
The best this KBO model could do on the blogs corpus was about x% accuracy.  We expect the accuracy to vary $\pm y$ with 90% confidence.  It would be interesting to compare the results of this analysis process with the news and twitter corpora.  Before attempting to analyze the other corpora in this fashion however, some work to speed up the computations would need to be done because it took a couple of weeks for my systems to crunch through all the prediction calculations.
