...
date: "December 13, 2016 (initial release)"
...
Discuss using cross entropy...

The best this KBO model could do on the blogs corpus was about 18% accuracy.  We expect the accuracy to vary between `r pi_low` and `r pi_high` with 95% confidence.  It would be interesting to compare the results of this analysis process with the news and twitter corpora.  Before attempting to analyze the other corpora in this fashion however, some work to speed up the computations would need to be done because it took a couple of weeks for my systems to crunch through all the prediction calculations.

While writing up this project, learned that accuracy is not the standard way to access the performance of a language model like this.  A better measure to use for optimizing the discount rates would be something like the [cross entropy](https://en.wikipedia.org/wiki/Cross_entropy#Cross-entropy_error_function_and_logistic_regression).
